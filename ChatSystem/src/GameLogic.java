...
public class GameLogic
{
     ...
     * Default constructor
     */
    public GameLogic(Map map, HumanPlayer player, Bot bot, Socket args) throws IOException
    {
        this.map = map;
        this.player = player;
        this.bot = bot;
        this.args = args;
        outputStream = args.getOutputStream();
    ...
    protected String pickup()
    {
        ...
        if(entireMap[playerPosition[1]][playerPosition[0]] == 'G') // Check if player position is on top of gold
        {
            ...
            player.receiveGold();
            writer.println("SUCCESS. " + "Gold Owned: " + gold());
        }
        else
        {
            writer.println("FAIL. " + "Gold Owned: " + gold());
        }
        return null;
    }
    ...
    protected void quitGame()
    {
        ...
        {
            writer.println("GAME WON");
            player.maxGold();
            gameRunning();
        }
        else
        {
            writer.println("GAME LOST"); // LOSS CONDITION
            player.maxGold();
            gameRunning();
        }
    }
     ...
     * @param args
     */
    public static void main(Socket args) throws Exception
    {
        Map map = new Map();
        HumanPlayer player = new HumanPlayer();
        Bot bot = new Bot();
        GameLogic logic = new GameLogic(map, player, bot, args); // Creating a GameLogic object

        logic.spawnPlayers();
        int counter = logic.playerCounter;

        InputStream inputStream = args.getInputStream(); // Collect stream of data sent FROM server
        BufferedReader reader = new BufferedReader(new InputStreamReader(inputStream));


        writer.println("Game Start!");
        writer.println("Which map would you like to use?");
        writer.println("1) Default Map");
        writer.println("2) Small Dungeon of Doom");
        writer.println("3) Medium Dungeon of Disaster");
        writer.println("4) Large Dungeon of Despair");
        ...
        switch (input)
        {
            ...
            case "4":
                ...
                break;
            default:
                writer.println("Invalid, therefore default has been chosen");
                break;
        }
        writer.println(map.getMapName());
        do
        {
            ...
            if(counter % 2 == 0) // Player turn
            {
                ...
                {
                    case "hello":
                        writer.println("Gold to win: " + logic.goldRequired());
                        break;
                    case "gold":
                        writer.println("Gold owned: " + logic.gold());
                        break;
                    case "move":
                        try
                        {
                        ...
                        break;
                    case "pickup":
                        writer.println("Picking...");
                        logic.pickup();
                        break;
                    case "look":
                        writer.println(logic.look());
                        break;
                    case "quit":
                        logic.quitGame();
                        break;
            ...
            else // Bot turn
            {
                ...
                {
                    writer.println("It's coming for you...");
                    bot.increaseCounter();
                }
                ...
                else if(bot.botChase == true)
                {
                    ...
                    {
                        writer.println("It has lost sight of you...");
                        bot.resetState();
                    }
                    else
                    {
                        writer.println("BOT IS GOING TO CHASE IN: " + (bot.counter() * -1 + 4) + " TURNS");
                        bot.increaseCounter();
                    }
                }
                /* Random movement */
            ...
            {
                writer.println("You have been caught...");
                player.maxGold();
            }
        } while(logic.gameRunning() == true); //Loop until game ends
        writer.println("Game has ended!");
        // Closing client
        args.close();
    }
}